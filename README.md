...
Wallet actions are exposed through the `radius-wallet` Hermes plugin (toolset `radius_wallet`) and internally call `/app/scripts/radius/cmd-wallets.mjs`, `cmd-balance.mjs`, `cmd-send.mjs` (supports `--asset=sbc|rusd`), and `cmd-fund.mjs` (`--wallet=all` plus backward-compatible `both`).
...
### Plugins (tool registration)

Radius wallet operations are now registered via a local Hermes plugin bundled in this repo at `plugins/radius-wallet/`. On boot, entrypoint installs repo plugins into `$HERMES_HOME/plugins/` so Hermes can load them at startup.

The plugin registers tools for:
- listing wallets
- showing/switching default wallet
- funding one/all wallets
- checking wallet balances
- sending SBC/RUSD transfers
